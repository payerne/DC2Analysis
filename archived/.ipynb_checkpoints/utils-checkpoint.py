...
def _is_complete(cl, r_limit, cosmo):
    ...
    angle = np.sqrt(((cl.galcat['dec'] - cl.dec)**2 + (cl.galcat['ra'] - cl.ra)**2*np.cos(cl.dec*np.pi/180)))*np.pi/180
    
    phi = np.arctan((cl.galcat['dec'] - cl.dec)/((cl.ra - cl.galcat['ra'])*np.cos(cl.dec*np.pi/180)))
